...
def helper(dis):
    ...
    med = med.values.flatten().tolist() if not med.empty else ["No medications available."]

    die = diets[diets['Disease'] == dis]['Diet']
    die = die.values.flatten().tolist() if not die.empty else ["No diet information available."]

    wrkout = workout[workout['disease'] == dis]['workout']
    wrkout = wrkout.values.flatten().tolist() if not wrkout.empty else ["No workout recommendations available."]
...
symptoms_dict = {
    ...
    'dehydration': 29, 'indigestion': 30, 'headache': 31, 'yellowish_skin': 32, 'dark_urine': 33,
...
def get_predicted_value(patient_symptoms):
    ...
    # Ensure symptoms are correctly mapped
    for symptom in patient_symptoms:
        if symptom in symptoms_dict:
            input_vector[symptoms_dict[symptom]] = 1
            print(f"Symptom '{symptom}' mapped to index {symptoms_dict[symptom]}")  # Debugging output
        else:
            print(f"Symptom '{symptom}' not recognized.")  # Debugging output for unrecognized symptoms

    print(f"Input Vector: {input_vector}")  # Debugging output

    # Predict disease index
    predicted_index = svc.predict([input_vector])[0]
    print(f"Raw predicted index: {predicted_index}")  # Debugging output for raw prediction

    # Ensure correct mapping from index to disease
...
def predict():
    if request.method == 'POST':
        symptoms = request.form.get('symptoms', '').strip()  # Get symptoms input safely
        user_symptoms = [s.strip().lower() for s in symptoms.split(',') if s.strip()]  # Clean input

        print(f"User  Symptoms: {user_symptoms}")  # Debugging output

        # Ensure that user_symptoms is passed to the function
        predicted_disease = get_predicted_value(user_symptoms)

        # Fetch details using helper function
